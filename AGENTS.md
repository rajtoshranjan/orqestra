...
The agent acts on the graph only through its grounded operations and the frontend service registry and canvas helpers; never give it a private mutation path. Mutating operations go through the same `checkConnection` / `checkParent` rules a human edit does (`client/src/utils/graph-rules.ts`) — one rule, two callers. Read-only operations (`list_services`, `get_service`, `query_graph`) are answered server-side from the stored catalog and the posted graph; never route a read back through the client.
...
- `docs/ai-agent.md` — how the AI agent works: surfaces, run loop, operations, risk model, LLM providers
