...
from agent.risk import classify_operation_risk
...
class RiskTests(SimpleTestCase):
    def test_remove_requires_confirmation(self):
        self.assertEqual(
            classify_operation_risk("remove", {"target_id": "n1"}), RiskLevel.CONFIRM
        )
    ...
    def test_read_only_operations_are_safe(self):
        self.assertEqual(classify_operation_risk("query_graph", {}), RiskLevel.SAFE)
