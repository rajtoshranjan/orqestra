...
from agent.llm.types import LLMMessage, Role, ToolCallBlock, ToolResultBlock, ToolSpec
...
class MapperTests(SimpleTestCase):
    ...
    def test_assistant_tool_call_maps_to_tool_use(self):
        messages = [
            LLMMessage(
                role=Role.ASSISTANT,
                content=[ToolCallBlock(id="tc_1", name="add_resource", input={"x": 1})],
            )
        ]
    ...
    def test_tool_role_maps_to_user_with_tool_result(self):
        messages = [
            LLMMessage(
                role=Role.TOOL,
                content=[ToolResultBlock(tool_call_id="tc_1", content="done")],
            )
        ]
